...
figure
semilogy(10*log(2.5./(N0)/3),BER_coded_QAM)
...
%-------------------------------------------------------------
figure
semilogy(10*log(1./N0/3),BER_coded_QPSK)
xlabel('SNR (dB)'); ylabel('BER')
hold on
...
% 3) IFFT:
%---------
QAM_IFFT_coded = ifft(QAM_data_mapped_all_coded,64);     % *sqrt(3) eq to compare with -2/sqrt(3) in the demapper
...
for j = 1:length(N0)
    ...
    QAM_AWGN_FFT_coded= [real(QAM_AWGN_FFT_coded(:)) imag(QAM_AWGN_FFT_coded(:))]*sqrt(3);
    %=>Frequency selective:
    ...
    QAM_Freq_FFT_coded= [real(QAM_Freq_FFT_coded(:)) imag(QAM_Freq_FFT_coded(:))]*sqrt(3);   
...
%% Show the output:
figure
semilogy(10*log(2.5./N0/3),BER_QAM_Freq_coded)
xlabel('SNR (dB)'); ylabel('BER')
hold on
...
figure
semilogy(10*log(2.5./N0/3),BER_QAM_AWGN_coded)
xlabel('SNR (dB)'); ylabel('BER')
hold on
...
% 3) IFFT:
%---------
QPSK_IFFT_coded = ifft(QPSK_data_mapped_all_coded,64); % *sqrt(3) eq to compare with -2/sqrt(3) in the demapper
...
for j = 1:length(N0)
    ...
    QPSK_AWGN_FFT_coded= [real(QPSK_AWGN_FFT_coded(:)) imag(QPSK_AWGN_FFT_coded(:))]*sqrt(3);
    %=>Frequency selective:
    ...
    QPSK_Freq_FFT_coded= [real(QPSK_Freq_FFT_coded(:)) imag(QPSK_Freq_FFT_coded(:))]*sqrt(3);
...
%% Show the output:
figure
semilogy(10*log(1./N0/3),BER_QPSK_Freq_coded)
xlabel('SNR (dB)'); ylabel('BER')
hold on
...
figure
semilogy(10*log(1./N0/3),BER_QPSK_AWGN_coded)
xlabel('SNR (dB)'); ylabel('BER')
hold on
